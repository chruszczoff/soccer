...
def get_upcoming_matches(league_name):
    ...
    headers = {"x-apisports-key": API_KEY}
    url = f"{BASE_URL}fixtures?league={league_id}&season=2024&date={tomorrow}"
    response = requests.get(url, headers=headers)
    return response.json()["response"]
...
def calculate_accuracy(data):
    accuracies = {"Premier League": {"correct": 0, "total": 0}, "La Liga": {"correct": 0, "total": 0}, "Ekstraklasa": {"correct": 0, "total": 0}, "Overall": {"correct": 0, "total": 0}}
...
def index():
    ...
    for league in LEAGUE_IDS.keys():
        ...
        if not matches:
            predictions.append({"league": league, "message": "Brak meczów na jutro."})
        else:
            for match in matches:
